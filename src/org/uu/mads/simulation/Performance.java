...
public class Performance {
	...
	public void addDelay(final Duration delay) {
	...
	public Duration calculateAveragePunctuality() {
		...
		if (this.total_delays != 0) {
			this.average_delay = this.total_delay_time.dividedBy(this.total_delays);
		} else {
			this.average_delay = Duration.ZERO;
		}
		...
		System.out
				.println("Average delay time: (total delay amount / total delays): " + this.average_delay.getSeconds());
